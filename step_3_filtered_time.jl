...
# Multi-stage lidar timestamp filter (no stability filter):
#   Stage 1: time-of-day window + month/year
#   Stage 2: clouds vs PBL relationship
...
#
# Output: filtered_time_lidar_<condition>.csv with columns
#   day, month, year, time_s, pbl_h, cloud_h, rh_h
# (note: no stability column since stability filter is dropped)


@everywhere function one_month(df_time, df_pbl, df_clouds, df_precip, df_rh,
                                time_in_h, time_out_h, month_flag, condition_flag)
    ...
    # === PBL height lookup (no filter, just attach the value) ===
    pbl_for_lidar = []
    ...
    return N_total, N_clouds, N_prcpt, N_rh,
           [l_day, l_month, l_year, l_time, set_pbl_h, set_cld_h, set_rh_h]
end


@everywhere function compute_one_month(d, df_time, df_pbl, df_clouds, df_precip, df_rh, condition_flag)
    ...
    for t in 0:1.0:23.0
        N1, N2, N3, N4, time_info = one_month(df_time, df_pbl, df_clouds, df_precip, df_rh,
                                              t, t + 1.0, month_flag, condition_flag)
        push!(true_time, t)
        push!(true_N1, N1)
        push!(true_N2, N2)
        push!(true_N3, N3)
...
function main(condition_flag)
    ...
    results = pmap(d -> compute_one_month(d, df_time, df_pbl, df_clouds, df_precip, df_rh, condition_flag), months_all)
    ...
    for (i, (d, true_time, true_N1, true_N2, true_N3, true_N4, time_info)) in enumerate(results)
        ...
        bar!(plt, true_time, true_N2, bar_width=0.6, alpha=0.6, label="Clouds",           color=c2, subplot=i, ylim=(0, 150), xlim=[-1, 25])
        bar!(plt, true_time, true_N4, bar_width=0.3, alpha=1.0, label="Precipitation+RH", color=c3, subplot=i, ylim=(0, 150), xlim=[-1, 25])
    ...
    output_csv = joinpath(PATH_output_txt, "filtered_time_lidar_$(condition_flag).csv")
    CSV.write(output_csv, df_out)
    println(output_csv)

    output_png = joinpath(PATH_output_png, "monthly_time_vs_N_panel_$(condition_flag).png")
    savefig(output_png)
    println(output_png)
end
